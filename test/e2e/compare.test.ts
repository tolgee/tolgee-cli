...
const CODE_UNORDERED = join(FIXTURES_PATH, 'codeProjectReactUnordered', 'App.tsx');
...
it('detects keys that no longer exist', async () => {
  ...
  expect(out.code).toBe(0);
  expect(out.stdout).toContain('out of sync');
  expect(out.stdout).toContain('2 keys exist on Tolgee but were not found in your code project');
  // Matching \n is important to ensure it properly understood it is not a namespaced string
...
it('handles namespaces properly', async () => {
  ...
  expect(out.stdout).toContain('3 keys exist on Tolgee but were not found in your code project');
...
it.only('prints keys sorted in alphabetical order', async () => {
  ...
  expect(out.stdout.indexOf('section-content')).toBeLessThan(out.stdout.indexOf('section-title'));
  expect(out.stdout.indexOf('section-title')).toBeLessThan(out.stdout.indexOf('copyright-notice'));
  expect(out.stdout.indexOf('copyright-notice')).toBeLessThan(out.stdout.indexOf('privacy-policy'));
  expect(out.stdout.indexOf('privacy-policy')).toBeLessThan(out.stdout.indexOf('terms-of-service'));
  expect(out.stdout.indexOf('terms-of-service')).toBeLessThan(out.stdout.indexOf('welcome'));
  ...
  expect(out.stdout.indexOf('bird-name')).toBeLessThan(out.stdout.indexOf('bird-sound'));
  expect(out.stdout.indexOf('bird-sound')).toBeLessThan(out.stdout.indexOf('cat-name'));
  expect(out.stdout.indexOf('cat-name')).toBeLessThan(out.stdout.indexOf('cat-sound'));
  expect(out.stdout.indexOf('cat-sound')).toBeLessThan(out.stdout.indexOf('dog-name'));
  expect(out.stdout.indexOf('dog-name')).toBeLessThan(out.stdout.indexOf('dog-sound'));
}, 30e3);
