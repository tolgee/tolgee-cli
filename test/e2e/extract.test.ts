...
it('prints all the strings and warnings from test project', async () => {
  const out = await run([ 'extract', 'print', '--extractor', 'react', CODE_PROJECT_ERR_MATCH ])
...
it('prints all the checking information from test project (without error)', async () => {
  const out = await run([ 'extract', 'check', '--extractor', 'react', CODE_PROJECT_MATCH ])
...
it('prints all the checking information from test project (with error)', async () => {
  const out = await run([ 'extract', 'check', '--extractor', 'react', CODE_PROJECT_ERR_MATCH ])
...
it('spits GitHub Workflow Commands when it detects GH Actions env', async () => {
  const out = await run([ 'extract', 'check', '--extractor', 'react', CODE_PROJECT_ERR_MATCH ], { CI: 'true', GITHUB_ACTIONS: 'true' })
