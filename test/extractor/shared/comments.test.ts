...
it('notifies ignore comment has been found', () => {
  ...
  expect(callback).toHaveBeenCalledWith({ type: 'MAGIC_COMMENT', kind: 'ignore', line: 1 })
})
...
it('notifies key comment has been found', () => {
  ...
  expect(callback).toHaveBeenCalledTimes(1)
  expect(callback).toHaveBeenCalledWith({ type: 'MAGIC_COMMENT', kind: 'key', keyName: 'meow', line: 1 })
})

it('extracts json5 struct from comment', () => {
  send({ type: 'COMMENT', data: ' @tolgee-key { key: "meow", ns: "nya" }', line: 1 })

  expect(callback).toHaveBeenCalledTimes(1)
  expect(callback).toHaveBeenCalledWith({ type: 'MAGIC_COMMENT', kind: 'key', keyName: 'meow', namespace: 'nya', line: 1 })
})
...
it('doesn\'t parse escaped json5', () => {
  ...
  expect(callback).toHaveBeenCalledTimes(1)
  expect(callback).toHaveBeenCalledWith({ type: 'MAGIC_COMMENT', kind: 'key', keyName: '{key:"t"}', line: 1 })
})
...
it('warns on invalid json5', () => {
  ...
  expect(callback).toHaveBeenCalledTimes(1)
  expect(callback).toHaveBeenCalledWith({ type: 'WARNING', kind: 'W_MALFORMED_KEY_OVERRIDE', line: 1 })
})
...
it('warns on missing key in json5', () => {
  ...
  expect(callback).toHaveBeenCalledTimes(1)
  expect(callback).toHaveBeenCalledWith({ type: 'WARNING', kind: 'W_INVALID_KEY_OVERRIDE', line: 1 })
})
